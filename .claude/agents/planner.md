...
> **เรื่องโมเดล/ต้นทุน:** agent นี้ตั้งไว้ที่ `sonnet` เพื่อประหยัดโทเคน — พอสำหรับฟีเจอร์ที่ pattern
> ชัด (CRUD, auth, list) ถ้าเป็นงานที่ต้อง**ตัดสินใจ architecture ใหม่จริง ๆ** (เลือก pattern ระดับระบบ,
> trade-off ที่ผิดแล้ว cascade ทั้งโปรเจกต์) ให้ main agent สั่ง planner ด้วย opus เป็นราย ๆ ไป
